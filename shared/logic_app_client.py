...
class LogicAppClient:
    ...
    def send_alert(
        ...
        Returns:
            True if successful
        """
        # Build enhanced payload with rich AI analysis
        payload = {
            "timestamp": datetime.utcnow().isoformat() + "Z",
            "source": "Azure Anomaly Detection System",
            "version": "2.0",
            ...
            "analysis": {
                ...
                "businessImpact": analysis.get("businessImpact", "minimal"),
                "cascadeRisk": analysis.get("cascadeRisk", 0.0),
                "reasoning": analysis.get("reasoning", "")
            },
            
            # Predictions
            "predictions": {
                "nextValue": analysis.get("nextValue", current_value),
                "timeToNextAnomaly": analysis.get("timeToNextAnomaly"),
                "forecast": analysis.get("prediction", {})
            },
            
            # Actions
            "recommendedActions": analysis.get("recommendedActions", ["monitor"]),
            "action": analysis.get("action", "monitor")  # Backward compatibility
        }
    ...
    def _get_threshold_for_metric(self, metric_name: str) -> float:
        """Get threshold value for metric (from environment variables to match prefilter)"""
        thresholds = {
            "performance_counters_processor_time": float(os.getenv("THRESHOLD_CPU_CRITICAL", "80.0")),
            "performance_counters_memory": float(os.getenv("THRESHOLD_MEMORY_CRITICAL", str(104857600))),  # 100MB
            "requests_duration": float(os.getenv("THRESHOLD_RESPONSE_TIME_CRITICAL", "1000.0")),
            "requests_failed": float(os.getenv("THRESHOLD_FAILURES_STATIC_CRITICAL", "0.0"))
        }
