...
METRICS_CONFIG = {
    ...
    "request_failed": {
        ...
        "unit": "count",
        "category": "AppRequests",
        "description": "Total number of failed HTTP requests per minute (aggregated for spike detection)"
    },
    ...
    "exception_count": {
        ...
        "description": "Total number of exceptions per minute (aggregated for spike detection)"
    },
...
def create_metrics_service() -> Optional[MetricsQueryClient]:
    ...
    """
    try:
        # Try environment variable first, then fallback to hardcoded value
        workspace_id = os.getenv("APPINSIGHTS_WORKSPACE_ID", WORKSPACE_ID)
        if not workspace_id:
